...
import { lessonsData, parentData, studentData, teacherData } from '@/app/lib/data'
...
const LessonListpage = () => {
  const renderRow = (item) => (
    <tr key={item.id} className='border-b border-gray-200 even:bg-slate-50 text-sm hover:bg-gray-300'>
        <td className='hidden md:table-cell p-4 font-bold'>{item.lesson}</td>
      <td className='hidden md:table-cell gap-4 p-4 '>{item.subjectName}</td>
